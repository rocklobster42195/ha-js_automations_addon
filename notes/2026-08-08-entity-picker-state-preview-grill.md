...
- Entity *state* data (`state`, `attributes`) lives in a **separate** global, `window.cachedEntities: JsaHaState[]`, populated by `fetchAllStatesDeduped()` (`ha-entity-cache.ts`) — currently only pulled in by `<status-bar-header-actions>` and `<status-bar>` (a handful of configured entities), not guaranteed populated/fresh for the *entire* entity list when the picker opens.
