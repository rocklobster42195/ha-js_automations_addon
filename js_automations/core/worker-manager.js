...
class WorkerManager extends EventEmitter {
  ...
  async syncServiceDefinitions() {
    ...
    try {
      ...
      this.emit('log', {
        source: 'System',
        message: `ServiceMap generated: ${Object.keys(services).length} domains mapped.`,
        level: 'debug',
      });
    } catch (e) {
  ...
  setSettings(newSettings) {
    ...
    this.emit('log', {
      source: 'System',
      message: `Settings updated. Restart protection: ${this.settings.restart_protection_count} starts in ${this.settings.restart_protection_time / 1000}s.`,
      level: 'debug',
    });
  }
  setStorageDir(dir) {
    ...
    if (fs.existsSync(targetPath)) {
      try {
        ...
        if (resolution === 'node' || resolution === 'node10') {
          ...
          this.emit('log', {
            source: 'System',
            message: 'Upgraded deprecated moduleResolution in tsconfig.json to Node16.',
            level: 'info',
          });
        }
      } catch (e) {
  ...
  async unregisterEntity(filename, entityId) {
    ...
    if (this.mqttManager && this.mqttManager.isConnected) {
      this.emit('log', {
        source: 'System',
        message: `[WorkerManager] Removing entity ${entityId} via MQTT`,
        level: 'debug',
      });
  ...
  async republishNativeEntities(onlyIfMissing = false) {
    ...
    if (count > 0) {
      this.emit('log', {
        source: 'System',
        message: `Republished ${count} entities (Skipped ${total - count} healthy ones).`,
        level: 'debug',
      });
    }
    ...
    for (const scriptFilename of this.workers.keys()) {
      const slug = path
        .basename(scriptFilename, path.extname(scriptFilename))
        .toLowerCase()
        .replace(/\s+/g, '_')
        .replace(/[^a-z0-9_]/g, '');
  ...
  async removeScriptEntities(filename) {
    ...
    this.emit('log', {
      source: 'System',
      message: `[WorkerManager] Cleaning up metadata for ${filename}`,
      level: 'debug',
    });
    ...
    if (this.scriptEntityMap.has(actualKey)) {
      ...
      for (const entityId of entities) {
        ...
        if (this.mqttManager && this.mqttManager.isConnected) {
          ...
          // One exact topic: object_id for ha.register, unique_id for @expose, derivedObjectId as fallback
          const topicId = payload?.object_id || payload?.unique_id || derivedObjectId;
          this.mqttManager.publish(`${this.mqttManager.discoveryPrefix}/${domain}/${topicId}/config`, '', {
            retain: true,
          });
          // Clear retained state/attributes topic
          const stateTopic = payload?.state_topic || `jsa/${domain}/${derivedObjectId}/data`;
  ...
  async _sweepOrphanedDynamicEntities(filename, { skipWorkerCheck = false } = {}) {
    ...
    this.emit('log', {
      source: 'System',
      message: `[WorkerManager] Running sweep for orphaned dynamic entities in ${filename}`,
      level: 'debug',
    });
    ...
    for (const entityId of knownEntities) {
      ...
      if (!currentRunEntities.has(entityId)) {
        this.emit('log', {
          source: 'System',
          message: `[WorkerManager] Entity ${entityId} is no longer requested by ${filename}. Removing from Home Assistant.`,
          level: 'debug',
        });
  ...
  startScript(filename) {
    ...
    if (recentRestarts.length >= this.settings.restart_protection_count) {
      this.emit('log', {
        source: 'System',
        message: `🛑 Script '${name}' stopped due to excessive restart rate (>${this.settings.restart_protection_count} restarts in ${this.settings.restart_protection_time / 1000}s).`,
        level: 'error',
      });
    ...
    this.emit('log', {
      source: 'System',
      message: `[WorkerManager] Scheduled entity sweep for ${scriptId} in 10s`,
      level: 'debug',
    });
    ...
    try {
      ...
      /* ignore read error, file existence checked above */
    }

    // Determine Language
    const language =
      this.settings.ui_language && this.settings.ui_language !== 'auto'
    ...
    worker.on('message', async (msg) => {
      try {
        ...
        if (msg.type === 'create_entity') {
          this.emit('log', {
            source: 'System',
            message: `[WorkerManager] Received ha.register for '${msg.entityId}' from ${name}`,
            level: 'debug',
          });
        ...
        if (msg.type === 'remove_entity') {
          if (this.protectedEntities.get(scriptMeta.filename)?.has(msg.entityId)) {
            this.emit('log', {
              source: 'System',
              message: `[WorkerManager] Ignoring ha.unregister('${msg.entityId}') — entity is declared via @expose and is managed automatically.`,
              level: 'warn',
            });
          } else {
            this.emit('log', {
              source: 'System',
              message: `[WorkerManager] Received ha.unregister for '${msg.entityId}' from ${name}`,
              level: 'debug',
            });
        ...
        if (msg.type === 'install_card' && this.cardManager) {
          ...
          if (devMode) {
            this.emit('log', {
              ...
              level: 'warn',
              message:
                '[Card] @card dev is set — card is preview-only and will NOT be installed to Lovelace. ' +
                'Change @card dev → @card in the script metadata to enable real installation.',
            });
          }
          this.cardManager
            .installCard(fullPath, { ...msg.options, devMode })
            .then((url) => {
              if (!devMode) {
            ...
            .catch((err) =>
              worker.postMessage({ type: 'install_card_response', callId: msg.callId, error: err.message })
            );
        ...
        if (msg.type === 'get_statistics') {
          ...
          (connector
            ...
            .catch((err) =>
              worker.postMessage({ type: 'get_statistics_response', callId: msg.callId, error: err.message })
            );
        }

        // ha.renderTemplate()
        if (msg.type === 'render_template') {
          (this.haConnector ? this.haConnector.renderTemplate(msg.template) : Promise.resolve(null))
            .then((result) => worker.postMessage({ type: 'render_template_response', callId: msg.callId, result }))
            .catch((err) =>
              worker.postMessage({ type: 'render_template_response', callId: msg.callId, error: err.message })
            );
        }
        ...
        if (msg.type === 'get_calendar_events') {
          (this.haConnector
            ...
            .catch((err) =>
              worker.postMessage({ type: 'get_calendar_events_response', callId: msg.callId, error: err.message })
            );
        }

        // ha.getTodoItems()
        if (msg.type === 'get_todo_items') {
          (this.haConnector ? this.haConnector.getTodoItems(msg.entityId) : Promise.resolve([]))
            .then((result) => worker.postMessage({ type: 'get_todo_items_response', callId: msg.callId, result }))
            .catch((err) =>
              worker.postMessage({ type: 'get_todo_items_response', callId: msg.callId, error: err.message })
            );
        }
        ...
        if (msg.type === 'webhook_register') {
          if (!this.webhookManager) {
            ...
            try {
              this.webhookManager.register(msg.id, {
                method: msg.method,
                noAuth: msg.noAuth,
    ...
    worker.on('exit', (code) => {
      if (this.workers.get(scriptMeta.filename) === worker) {
        ...
        this.activeRunEntities.delete(scriptMeta.filename);
        this.eventTypeSubscriptions.forEach((filenames) => filenames.delete(scriptMeta.filename));
        ...
        if (dirtyKeys && dirtyKeys.size > 0) {
          ...
          this.emit('log', {
            source: 'System',
            message: `[WorkerManager] "${scriptMeta.filename}" exited with unsaved persistent state: ${keys}`,
            level: 'warn',
          });
        }
      ...
      worker.on('message', async (msg) => {
        ...
        if (msg.type === 'remove_entity') {
          ...
          if (this.protectedEntities.get(ownerFilename)?.has(msg.entityId)) {
            logs.push({
              level: 'warn',
              message: `Ignoring ha.unregister('${msg.entityId}') — entity is declared via @expose and is managed automatically.`,
            });
          } else {
  ...
  setEntityIdAlias(haEntityId, userEntityId) {
    if (haEntityId && userEntityId && haEntityId !== userEntityId) {
      ...
      this.emit('log', {
        source: 'System',
        message: `[WorkerManager] Entity alias registered: ${haEntityId} → ${userEntityId}`,
        level: 'debug',
      });
  ...
  clearEntityIdAlias(userEntityId) {
    for (const [haId, userId] of this.entityIdAliases.entries()) {
      if (userId === userEntityId) {
        ...
        this.emit('log', {
          source: 'System',
          message: `[WorkerManager] Cleared stale alias: ${haId} → ${userEntityId}`,
          level: 'debug',
        });
        break;
      }
  ...
  stopScript(filename, reason = 'stopped by user') {
    ...
    if (worker) {
      ...
      setTimeout(() => {
        ...
        this.emit('log', {
          source: 'System',
          message: `[WorkerManager] "${filename}" did not stop gracefully — force-killing.`,
          level: 'warn',
        });
  ...
  shutdown() {
    ...
    if (this.saveRegistryTimer) {
      ...
      this.emit('log', {
        source: 'System',
        message: `[WorkerManager] Force saving registry on shutdown...`,
        level: 'debug',
      });
      this._performSaveRegistry();
    }
