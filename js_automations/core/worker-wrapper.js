  ...
  Module._load = function (request, parent) {
    if (_scriptExecuting) {
      ...
      const isSystemInternal = SYSTEM_INTERNAL_PACKAGES.some(
        (pkg) => parentFile.includes(`node_modules/${pkg}`) || parentFile.includes(`node_modules\\${pkg}`)
      );
  ...
  parentPort.on('message', async (msg) => {
    ...
    if (msg.type === 'state_update') {
      if (msg.state) {
        ...
        // msg.state is null or undefined, so delete the entity
    ...
    if (msg.type === 'ha_event') {
      ...
      subscriptionCallbacks.forEach((sub) => {
        ...
        if (sub.filter && sub.filter !== 'any') {
          ...
          switch (sub.filter) {
            case 'ne':
              match = sub.threshold !== undefined ? valNew != compValStr : valNew !== valOld;
              break;
            case 'eq':
              match = sub.threshold !== undefined ? valNew == compValStr : valNew === valOld;
              break;
            case 'gt':
              match = isNum && nNew > compVal;
              break;
            case 'ge':
              match = isNum && nNew >= compVal;
              break;
...
const ha = {
  ...
  notify: (message, options = {}) => {
    ...
    if (persistent) {
      ...
      service = target ? (target.startsWith('notify.') ? target.slice(7) : target) : 'notify';
    }
...
ha.persistent = (key, defaultValue = {}) => {
  ...
  if (typeof defaultValue !== 'object' || defaultValue === null) {
    ...
    return {
      get value() {
        return ha.store.get(key) ?? defaultValue;
      },
      set value(v) {
        ha.store.set(key, v);
      },
  ...
  if (
    ...
    Array.isArray(target) !== Array.isArray(defaultValue)
  ) {
    target = defaultValue;
