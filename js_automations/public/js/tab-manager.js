...
function renderTabs() {
  ...
  openTabs.forEach((tabData, index) => {
    ...
    if (isCardTab) {
      ...
      const scriptFromList =
        typeof allScripts !== 'undefined' ? allScripts.find((s) => s.filename === tabData.filename) : null;
...
async function openCardTab(scriptFilename) {
  ...
  try {
    ...
    newTab.model.onDidChangeContent(() => {
      ...
      const isNowDirty = newTab.originalContent === null ? true : newTab.model.getValue() !== newTab.originalContent;
      if (newTab.isDirty !== isNowDirty) {
        newTab.isDirty = isNowDirty;
...
function updateToolbarUI(filename, icon, isDirty) {
  ...
  if (isCardTab) {
    ...
    if (toggleBtn) {
      ...
      toggleBtn.style.opacity = '0.1';
    }
    if (restartBtn) {
      restartBtn.disabled = true;
      restartBtn.style.opacity = '0.1';
    }
    if (editBtn) {
      editBtn.disabled = true;
      editBtn.style.opacity = '0.1';
    }
    if (dupBtn) {
      dupBtn.disabled = true;
      dupBtn.style.opacity = '0.1';
    }
    if (deleteBtn) {
  ...
  if (filename === 'System: Store' || filename === 'System: Settings') {
    ...
    const script = typeof allScripts !== 'undefined' ? allScripts.find((s) => s.filename === filename) : null;
    const isLib =
      script && script.path && (script.path.includes('/libraries/') || script.path.includes('\\libraries\\'));
...
function updateEditorMode(filename) {
  const isCardTab = filename.endsWith(CARD_TAB_SUFFIX);
  const script =
    !isCardTab && typeof allScripts !== 'undefined' ? allScripts.find((s) => s.filename === filename) : null;
  ...
  if (isCardTab) {
    ...
    banner.innerHTML =
      '<i class="mdi mdi-bookshelf" style="margin-right:6px;"></i> ' + i18next.t('library_mode_banner', { filename });
    container.insertBefore(banner, container.firstChild);
  }
}
  ...
  if (activeTab.type === 'blockly') {
    ...
    if (!blocklyWorkspaceHasTrigger(blocksState) && !isExposed) {
      const proceed = await window.confirmDialog.confirm(
        i18next.t('blockly_no_trigger_warning', {
