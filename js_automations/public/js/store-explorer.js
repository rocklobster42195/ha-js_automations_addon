...
function compareStoreEntries(a, b) {
  ...
  const getMeta = (item) =>
    item && typeof item === 'object' && 'owner' in item
      ? item
      : { owner: i18next.t('store.system_owner', { defaultValue: 'System' }), updated: 0, accessed: 0 };
  ...
  if (currentSort.column === 'key') {
    ...
    valA = (getMeta(itemA).owner || '').toLowerCase();
    valB = (getMeta(itemB).owner || '').toLowerCase();
  } else if (currentSort.column === 'updated') {
    valA = getMeta(itemA).updated || 0;
    valB = getMeta(itemB).updated || 0;
  } else if (currentSort.column === 'accessed') {
    valA = getMeta(itemA).accessed || 0;
    valB = getMeta(itemB).accessed || 0;
  }
...
async function loadStoreData() {
  ...
  try {
    ...
    const [storeRes, dirtyRes] = await Promise.all([apiFetch('api/store'), apiFetch('api/store/dirty')]);
    if (storeRes.ok) {
...
function renderStoreTable() {
  ...
  keys.forEach((key) => {
    ...
    row.dataset.key = key;
    if (isDirty)
      row.title = i18next.t('store.dirty_tooltip', { defaultValue: 'Unsaved in-memory changes (ha.persistent)' });
...
function buildStoreRowInnerHtml(key, item, isDirty, collapsed) {
  ...
  const meta =
    item && typeof item === 'object' && 'owner' in item
      ? item
      : { owner: i18next.t('store.system_owner', { defaultValue: 'System' }), updated: null, isSecret: false };
