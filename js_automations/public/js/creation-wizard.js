...
function renderCapPreview(el, data) {
  ...
  if (data.name)
    html += `<div class="cap-preview-name"><i class="mdi mdi-script-text"></i> <strong>${data.name}</strong></div>`;
  ...
  if (detected.length === 0) {
    ...
    html += visible
      .map((t) => {
        ...
        const cls = isWarn
          ? t === 'exec'
            ? 'cap-preview-item cap-preview-item-exec'
            : 'cap-preview-item cap-preview-item-warn'
          : 'cap-preview-item';
                ...
                <div class="wizard-grid">
                    ...
                    <div class="form-group hidden" id="wizard-group-template">
                        ...
                        <select id="wizard-template">
                            ${Object.keys(SCRIPT_TEMPLATES)
                              .map(
                                (k) =>
                                  `<option value="${k}" data-i18n="${SCRIPT_TEMPLATES[k].labelKey}">${i18next.t(SCRIPT_TEMPLATES[k].labelKey)}</option>`
                              )
                              .join('')}
                        </select>
                    </div>
...
async function openCreationWizard(mode = 'create', data = null) {
  ...
  if (window.updateUIWithTranslations)
    window.updateUIWithTranslations(document.getElementById('creation-wizard-modal'));
  ...
  if (mode === 'create') {
    ...
    if (data.includes)
      data.includes.forEach((i) => {
        if (!wizardIncludes.includes(i)) wizardIncludes.push(i);
      });
    renderWizardTags('includes');
  }
...
function handleFileSelect(input) {
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = async (e) => {
      try {
        const res = await apiFetch('api/scripts/preview', {
          method: 'POST',
...
function renderWizardTags(type) {
  if (type === 'npm') {
    ...
    if (!container) return;
    container.innerHTML = wizardNpmModules
      .map((pkg) => {
    ...
    if (!container) return;
    container.innerHTML = wizardIncludes
      .map((libName) => {
        const script =
          typeof allScripts !== 'undefined'
            ? allScripts.find((s) => s.filename === libName || s.filename === libName + '.js')
            : null;
...
async function executeWizardAction() {
  ...
  try {
    if (currentWizardTab === 'new') {
      ...
      let type = 'automation';
      let expose = null;
      if (wizardType === 'switch') {
        expose = 'switch';
      } else if (wizardType === 'button') {
        expose = 'button';
      } else if (wizardType === 'library') {
        type = 'library';
      }
      ...
      const payload = {
        ...
        description,
        extension,
        npmModules: wizardNpmModules.map((p) => p.name),
        includes: wizardIncludes,
      };
      ...
      if (wizardMode === 'edit') {
        ...
        if (!res.ok) {
          ...
          throw new Error(err.error || i18next.t('error_update_failed', { defaultValue: 'Update failed' }));
        }
        const data = await res.json();
        ...
        // CREATE or DUPLICATE
        payload.code =
          wizardMode === 'duplicate' && wizardDuplicateCode
            ? wizardDuplicateCode
            : SCRIPT_TEMPLATES['empty']
              ? SCRIPT_TEMPLATES['empty'].code
              : '';
        ...
        if (!res.ok) {
          const err = await res.json();
          throw new Error(err.error || i18next.t('error_create_failed', { defaultValue: 'Creation failed' }));
        }
        const data = await res.json();
        newFilename = data.filename;
      }
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || i18next.t('wizard_btn_upload') + ' fehlgeschlagen');
      }
      const data = await res.json();
      newFilename = data.filename;
    } else if (currentWizardTab === 'import') {
      ...
      if (!wizardImportPreviewed) {
        ...
        if (previewEl)
          previewEl.innerHTML = `<div class="cap-preview-loading"><i class="mdi mdi-loading mdi-spin"></i> ${i18next.t('cap_preview_loading')}</div>`;
        if (previewEl) previewEl.classList.remove('hidden');
        const res = await apiFetch('api/scripts/import', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ url, type, name, dryRun: true }),
        });
        if (!res.ok) {
          const err = await res.json();
          throw new Error(err.error || i18next.t('error_import_failed', { defaultValue: 'Import failed' }));
        }
      ...
      if (window.openOrSwitchToTab) {
        ...
        if (wizardMode === 'edit') {
          ...
          if (tab) {
            ...
            if (
              typeof activeTabFilename !== 'undefined' &&
              activeTabFilename === newFilename &&
              typeof updateToolbarUI === 'function'
            ) {
