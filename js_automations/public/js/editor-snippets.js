...
function resolveAndInsertFromCursor(mode) {
  ...
  const deleteRange = new monaco.Range(pos.lineNumber, col - matchedText.length, pos.lineNumber, col);
...
function _showRegisterPicker(def, matchedText, cursorPos) {
  ...
  for (const domain of REGISTER_DOMAINS) {
    ...
    btn.onclick = () => {
      ...
      const deleteRange = new monaco.Range(cursorPos.lineNumber, col - matchedText.length, cursorPos.lineNumber, col);
      editor.executeEdits('snippet-trigger', [{ range: deleteRange, text: '' }]);
      editor.setPosition({
...
function buildSnippetToolbar(container, mode = 'script') {
  ...
  const label = typeof i18next !== 'undefined' ? i18next.t('snippets_title', { defaultValue: 'Snippets' }) : 'Snippets';
  ...
  toggleBtn.onclick = (e) => {
    e.stopPropagation();
    _toggleSnippetDropdown(toggleBtn, mode);
  };
...
function _toggleSnippetDropdown(anchorBtn, mode = 'script') {
  ...
  for (const group of activeGroups) {
    ...
    for (const def of entries) {
      const btn = document.createElement('button');
      const label = typeof i18next !== 'undefined' ? i18next.t(def.labelKey, { defaultValue: def.id }) : def.id;
      btn.title = label;
...
function registerSnippetContextMenu(editorInstance) {
  for (const def of SNIPPET_REGISTRY) {
    ...
    editorInstance.addAction({
      id: `snip-${def.id}`,
      label: typeof i18next !== 'undefined' ? i18next.t(def.labelKey, { defaultValue: def.id }) : def.id,
