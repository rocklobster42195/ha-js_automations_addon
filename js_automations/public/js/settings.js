...
async function loadSettingsData(isBackgroundRefresh = false) {
  try {
    const [schemaRes, settingsRes] = await Promise.all([apiFetch('api/settings/schema'), apiFetch('api/settings')]);
...
function scrollToSection(id) {
  ...
  if (id === 'integration' || id === 'installer') {
    element =
      document.getElementById('settings-installer-anchor') || document.getElementById('settings-section-system');
  }
