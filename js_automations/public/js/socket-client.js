...
function initSocket() {
  ...
  const handleConnectionEstablished = () => {
    ...
    if (
      ...
      typeof STORE_TAB_ID !== 'undefined' &&
      activeTabFilename === STORE_TAB_ID &&
  ...
  window.socket.on('log', (d) => {
    if (typeof appendLog === 'function') appendLog(d);
  });
  window.socket.on('status_update', () => {
    if (typeof loadScripts === 'function') loadScripts();
  });
  window.socket.on('ha_event_stream', (d) => {
    if (typeof onHaEventStream === 'function') onHaEventStream(d);
  });
  window.socket.on('breakpoint_hit', (d) => {
    if (typeof onBreakpointHit === 'function') onBreakpointHit(d);
  });
  window.socket.on('breakpoint_continued', (d) => {
    if (typeof onBreakpointContinued === 'function') onBreakpointContinued(d);
  });
  window.socket.on('watch_update', (d) => {
    if (typeof onWatchUpdate === 'function') onWatchUpdate(d);
  });
  window.socket.on('inspect_snapshot', (d) => {
    if (typeof onInspectSnapshot === 'function') onInspectSnapshot(d);
  });
  window.socket.on('watch_clear', (d) => {
    if (typeof onWatchClear === 'function') onWatchClear(d);
  });
  window.socket.on('mqtt_message_stream', (d) => {
    if (typeof onMqttMessage === 'function') onMqttMessage(d);
  });
  window.socket.on('store_changed', (d) => {
    if (typeof onStoreChanged === 'function') onStoreChanged(d);
  });
  ...
  window.socket.on('integration_status', (data) => {
    ...
    if (
      data?.is_connected &&
      !wasConnected &&
      typeof loadHAMetadata === 'function' &&
      haData.areas.length === 0 &&
      haData.labels.length === 0
    ) {
      loadHAMetadata();
    }
  });
