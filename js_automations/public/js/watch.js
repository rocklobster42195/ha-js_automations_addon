...
function onWatchClear(data) {
  ...
  if (_watchRows.size === 0) {
    ...
    hint.textContent = i18next.t('devtools.watch_hint', {
      defaultValue: "No watch expressions active. Use ha.watch('label', () => expr) in a script.",
    });
...
function clearInspectList() {
  ...
  hint.className = 'watch-hint';
  hint.textContent = i18next.t('devtools.inspect_hint', {
    defaultValue: "No entries yet. Use ha.inspect('label', { vars }) in a script.",
  });
