...
function initEventInspector() {
  ...
  const fireBtnLabel =
    typeof i18next !== 'undefined' ? i18next.t('devtools.fire_event', { defaultValue: 'Fire' }) : 'Fire';
...
function renderEiTypeDropdown(term) {
  ...
  const matches = t ? EI_EVENT_TYPES.filter((e) => e.toLowerCase().includes(t)) : EI_EVENT_TYPES;
...
function rerenderEiBuffer() {
  ...
  if (!list) return;
  list.innerHTML = '';
  _eventBuffer.filter(matchesFilter).forEach(renderEventInspectorEntry);
}
...
function renderEiDropdown(term) {
  ...
  const t = (term || '').toLowerCase().trim();
  const matches = t ? entities.filter((e) => e.toLowerCase().includes(t)).slice(0, 80) : entities.slice(0, 80);
