...
const statusBar = {
  ...
  updateMqttIndicator(status) {
    ...
    if (status.connected) {
      ...
      const err = status.error
        ? i18next.t('settings.system.mqtt_error', { error: status.error })
        : i18next.t('settings.system.mqtt_disconnected');
      item.title = err;
    }
  },
  ...
  drawSparkline(canvas, data, options = {}) {
    ...
    data.forEach((v, i) => {
      ...
      if (options.thresholds) {
        if (v >= options.thresholds[2])
          color = '#ff5555'; // Rot
        else if (v >= options.thresholds[1])
          color = '#ffb86c'; // Orange
  ...
  renderHeaderActions(conf) {
    ...
    ['header_action_1', 'header_action_2', 'header_action_3'].forEach((key) => {
      ...
      if (!entityId) {
        this.headerActionEntities.push(null);
        return;
      }
  ...
  async triggerHeaderAction(entityId) {
    ...
    if (isButton) {
      ...
      if (btn)
        btn.style.color =
          cached?.state === 'on' ? 'var(--secondary-text-color, #777)' : 'var(--primary-color, #03a9f4)';
    } else {
      service = 'press';
    }
