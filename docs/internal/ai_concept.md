...
class AiService {
  constructor(settingsManager, haConnector) {
    ...
    this._apiTypes = fs.readFileSync(path.join(__dirname, '../core/types/ha-api.d.ts'), 'utf8');
    this._readme = fs.readFileSync(path.join(__dirname, '../../README.md'), 'utf8');
  }
  ...
  async generate(userPrompt, scriptContent = null, errorLog = null) {
    ...
    if (scriptContent) {
      messages.push({
        role: 'user',
        content: `Current script (for context):\n\`\`\`typescript\n${scriptContent}\n\`\`\``,
      });
    }
...
| `schedule('cron', cb)`               | `schedule('cron', cb)` _(identisch)_ |
| `log(msg)`                           | `ha.log(msg)`                        |
