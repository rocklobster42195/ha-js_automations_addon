...
### 6. Waiting for States (`ha.waitFor` / `ha.waitUntil`)
...
### 8. Setting States & Creating Sensors (`ha.update`)
...
### 9.1. Sending Notifications (`ha.notify`)
...
### 9.2. Actionable Notifications (`ha.ask`)
...
### 10. `ha.entity()` — Fluent Entity API

Get a typed, chainable handle for a single entity. `ha.entity(entityId)` returns an object with one async method per service available in that entity's domain (e.g. `turn_on`, `turn_off`, `toggle` for `light.*`; `open_cover`, `close_cover` for `cover.*`) — generated from the same service catalog as `ha.call()`, so arguments are autocompleted and type-checked.

```javascript
// Call a domain-specific method directly — same effect as ha.call('cover.open_cover', { entity_id: ... })
await ha.entity('cover.garage_door').open_cover();

// Pass service data, same shape as the second argument to ha.call()
await ha.entity('light.living_room').turn_on({ brightness: 150, rgb_color: [255, 0, 0] });
```
...
The handle also exposes the entity's live `state` and typed `attributes`, plus `getAttribute()` for a single value:

```javascript
const tv = ha.entity('media_player.tv');
if (tv.state === 'playing') {
  ha.log(`Now playing: ${tv.getAttribute('media_title')}`);
}
```

Use `ha.entity()` when you're acting on one known entity and want autocomplete for its domain's services. For bulk actions across a pattern of entities, use `ha.select()` below instead.

### 11. Entity Selectors (`ha.select`)
...
### 13. Automatic Persistent State (`ha.persistent`)
...
### 14. Global Error Handling (`ha.onError`)
...
### 15. HA Connection Status (`ha.isConnected` / `ha.onConnectionChange`)
