...
`handleReconnection()` is called on a fixed 5-second poll (`setInterval` in `server.ts`, guarded by an `isReconnecting` flag so overlapping calls don't stack), not triggered by a route or by the frontend detecting a drop. It reconnects the HA WebSocket, refreshes the system language, republishes `@expose` entities, and calls `republishNativeEntities()` — so all entities created via `ha.register()` (including device-grouped ones) are present again after an HA restart.
...
| `ha.action(name, handler)`                         | — (local only)         | Registers named handler in-worker; dispatched later via `card_action`; see §8    |
...
`ha.action()` is purely local to the worker — it just adds the handler to an in-memory `actionHandlers` Map (`worker-wrapper.ts`). No message is sent to WorkerManager at registration time; WorkerManager only gets involved later, when something actually triggers the action.

### Trigger Path 1 — Lovelace Card via HA Event Bus

Two distinct identifiers are involved, one per layer — the doc used to conflate them into a single "correlationId" flowing end to end, which isn't how it works:

```
Card calls __jsa__.callAction('refresh', payload)
  │  generates correlation_id (card-manager.ts:73)
  ▼
HA WebSocket: fire_event { event_type: 'jsa_action', data: { script, action, payload, correlation_id } }
  │
  ▼
Kernel's central HA event subscription catches 'jsa_action' (kernel.ts:571), dedupes by
correlation_id (30s window), calls workerManager.callAction(filename, action, payload)
  │
  ▼
WorkerManager.callAction() generates its own internal callId ("act_<n>", worker-manager.ts:1638),
tracks { resolve, reject, worker } in pendingActionCalls, sends:
worker.postMessage({ type: 'card_action', action, payload, callId })
  │
  ▼
Worker looks up the handler in actionHandlers, executes it, responds:
pp.postMessage({ type: 'action_response', callId, result })   (worker-wrapper.ts:648-664)
  │
  ▼
WorkerManager resolves the pendingActionCalls entry for that callId
  │
  ▼
Kernel fires the HA event: jsa_action_result { correlation_id, result }   (kernel.ts:579-583,
reusing the ORIGINAL correlation_id from step 1 — callId never leaves the backend)
  │
  ▼
Card's __jsa__.connect(hass) listener matches correlation_id, resolves the Promise
```

Pending `callAction()` calls are tracked in a Map on the card's `__jsa__` object; they resolve or reject after a **20-second** timeout (`card-manager.ts:94-96`).
...
Other backend code can call `workerManager.callAction(filename, action, payload)` directly — the same method the HA Event Bus path above ends up calling — without going through the HA Event Bus at all. Currently used for internal routing (e.g., MQTT command → action).
...
- **`TypeDefinitionGenerator`**: Generates `entities.d.ts` (both `HAEntities` and `GlobalStoreSchema`) for IntelliSense; see §14
...
```json
{
  ...
  "availability": [{ "topic": "jsa/status" }, { "topic": "jsa/script/<scriptSlug>/status" }],
...
After publishing the Discovery payload, an **ACK Poll** starts — a polling loop that queries the Entity Registry every 500 ms until the entity is correctly registered (max. 40 attempts ≈ 20 seconds; `entity-manager.ts:520-521` — doubled from an earlier 10s budget that wasn't enough under load, per the code comment there).
...
1. **Static analysis (UI)** — `core/capability-analyzer.ts` scans the script source via regex and detects used capabilities: `network`, `fs:read`, `fs:write`, `exec`, plus `webhook` (any `ha.onWebhook()` call) and `mqtt` (`ha.mqtt.publish()`/`.subscribe()` — distinct from generic `network`, used to drive the sidebar's "needs MQTT" warning dot). The script list shows badges for each capability. Badges are gray when declared and detected, amber for undeclared capabilities, red for undeclared `exec`.
2. **Runtime enforcement (Worker)** — `core/worker-wrapper.ts` blocks undeclared access at runtime via the `Module._load` hook + `globalThis.fetch` override (network/exec) and via `checkRead()`/`checkWrite()` in `ha.fs` (filesystem). `ha.http` is additionally gated behind `network` (line ~1603) and `ha.onWebhook()` behind `webhook` (line ~1673), on top of the module/fetch/fs-quota checks above.
